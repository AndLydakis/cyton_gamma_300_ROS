...
class JointStatePublisher():
    def __init__(self):
        ...
        servos = ['elbow_roll', 'elbow_pitch', 'elbow_yaw',
                  'shoulder_roll', 'shoulder_pitch',
        ...
        for parameter in parameters:
            ...
            if parameter.find(dynamixel_namespace) != -1 and parameter.find("/controller_joint_names") != -1:
                # self.joints.append(rospy.get_param(parameter))
                self.joints =(rospy.get_param(parameter))
        ...
        for joint in self.joints:
            ...
            servo = int(joint.replace("joint_", ""))
            print servo

            # self.joint_states[joint] = JointStateMessage(joint, 0.0, 0.0, 0.0)
            self.joint_states[servos[servo-1]] = JointStateMessage(servos[servo-1]+"_joint", 0.0, 0.0, 0.0)
            # self.controllers.append(dynamixel_namespace + servo + "_position_controller")
            self.controllers.append(dynamixel_namespace + servos[servo-1] + "_position_controller")
